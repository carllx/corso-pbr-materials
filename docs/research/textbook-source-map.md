...
6. [综合分析与教材组合策略建议](#6-综合分析与教材组合策略建议)
...
- **深度评估**：
  - *材质聚焦度*：**100% 聚焦于材质与纹理创作**，完全剔除了冗长的建模与拓扑步骤，直接提供预制好的高质量模型资产供学生进行材质训练。
  - *PBR 与理论*：Part 1 深入讲解 PBR 光学常识，Part 5 专门探讨不同渲染器着色差异，理论与实战结合极好。
  - *教学适配度*：**极高**。结构完全符合“理论 $\rightarrow$ 基础道具 $\rightarrow$ 复杂多材质机械 $\rightarrow$ 场景建筑 $\rightarrow$ 渲染导出”的循序渐进认知规律，非常适合作为本科实操主教材（Core Textbook Candidate）。
...
- **出版社 / 出版年份**：[Born Digital](https://www.borndigital.co.jp/)，2023/2024 年 (共 576 页)
- **完整核查目录 (TOC)**：
...
- **深度评估**：
  - *定位判断*：程序化/参数化材质领域的**殿堂级权威教材**。但由于其厚达近 600 页且包含大量数学与函数图逻辑，在 8 周本科通识/专业基础课中若全盘讲授会导致学生严重认知过载。建议将其作为教师备课与程序化材质演示的 **Teacher Reference** 或进阶选修参考。
...
- **作者**：来阳
- **出版社 / 出版年份**：[人民邮电出版社](https://www.ptpress.com.cn/)，2024/2026 年
...
- **深度评估**：
  - *优点*：软件版本新，结合了目前最主流的 Blender + Painter + UE 组合，材质案例非常符合国内学生习惯，包含 AI 与引擎展示。
  - *局限*：由于是一本涵盖建模到材质的“全流程实战书”，建模与拓扑占据了较多篇幅，导致其对 PBR 物理原理底层、Texel Density 计算、通道打包与复杂着色器逻辑的理论阐述较浅。
  - *定位建议*：非常适合作为国内机房实训的中文配套 **Practical Companion**，但不宜单独作为纯材质理论的主教材。
...
- **深度评估**：
  - *优点*：作为国内较早聚焦 Painter 的教材，其项目化实训框架被国内多所专科与本科高校采纳为参考书。
...
1. **零建模负担（Zero Modeling Overhead）**：
   CGMA《Character Texturing for Games in Substance》与 Gnomon《Texturing and Shading 2》等专业材质课程，**100% 为学生提供已经拓扑好、拆好 UV 的高质量标准资产模型**（如工业机械臂、复古收音机、写实角色头部）。课堂时间全部用于观察真实材质参考、理解通道与绘制纹理，绝不让学生在课上花费数周时间从零建模型。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Substance 3D Painter実践講座** | CafeGroup / Born Digital | 2025 | Painter 10.x | 资产贴图绘制、烘焙、PBR物理 | 深度 | 深度 | 涉及(Triplanar) | 无 | 涉及 | 间接 | **无 (提供现成资产)** | 游戏+影视通用 | 完整提供 | **Core Textbook Candidate** |
...
| **Blender+SP建模与材质实战** | 来阳 / 人民邮电出版社 | 2024/2026 | Blender+SP | 道具建模、UV、Painter实战 | 中等 | 深度 | 基础 | 无 | 无 | 涉及 (混元3D) | **较重 (含40%建模)** | 游戏为主 | 完整提供 | **Practical Companion** |
...
| **作例で学ぶ Designerの教科書** | もんしょ等 / Born Digital | 2023/2024 | Designer 13.x | 程序化2D纹理、.sbsar | 深度 | 接口结合 | **极其深入** | **完全核心** | 结合 | 间接 | **无** | 游戏+影视通用 | 完整提供 | **Teacher Reference** |
...
### 6.1 国内外教材的结构性差异
- **国外出版物特征（尤其是日本 Born Digital 与欧美行业课程）**：专业垂直度极高，**“材质课就是材质课”**，绝不掺杂大量建模教学，直接提供工业级模型资产，集中全部篇幅打透 PBR 原理、分层逻辑、烘焙算子与多渲染器差异。
- **国内出版物特征**：倾向于“大而全全流程”与“案例步骤驱动”，建模与基础操作占比过高，对 PBR 微表面物理推导、Texel Density 计算与引擎着色器架构的深度讲解相对欠缺。
...
### 6.3 建议采用的“复合教材体系 (Composite Portfolio Strategy)”

基于本科 8 周教学规律与专业性要求，**强烈建议不采用单一纸质教材，而是采用“核心实战教材 + 官方在线理论 + 中文实训伴侣”的复合组合策略**：

1. **核心教材骨架 (Core Textbook Candidate)**：
   - 引入 **《Substance 3D Painter実践講座》(Born Digital, 2025)** 作为课程主干案例与材质思考模型的蓝本（其“理论 $\rightarrow$ 基础道具 $\rightarrow$ 机械 $\rightarrow$ 建筑 $\rightarrow$ 跨引擎渲染”五段式结构极为科学）。
2. **官方理论底座 (Living Official Source)**：
   - 以 Adobe **《The PBR Guide (Part 1 & Part 2)》** 和 **Epic Games / Blender 官方 PBR 文档** 作为学生的必读理论纲领，保证物理原理的严谨性。
3. **中文实操伴随教材 (Practical Companion)**：
   - 指定来阳 **《Blender+Substance 3D Painter 建模与材质制作实战》(2024/2026)** 作为学生课后中文实操速查与机房实操伴随参考书，但教师在课堂上应严格屏蔽其建模篇章，直取其材质绘制与实训部分。
4. **程序化与教师参考 (Teacher References & Precedents)**：
   - 教师备课以 **《作例で学ぶ Substance 3D Designerの教科書》**、**《Real-Time Rendering, 4th》** 与 **CGMA 课程大纲** 为质量基准，确保教学视野贴合国际一线工业界。
